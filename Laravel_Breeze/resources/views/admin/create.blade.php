                    ...
                    <form method="POST" action="{{ route('admin.store') }}">
                        ...
                        <div class="py-2">
                            <label for="guard_name"
                                class="block font-medium text-sm text-gray-700{{ $errors->has('guard_name') ? ' text-red-400' : '' }}">{{ __('Guard Name') }}</label>
                            <input id="name"
                                class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block mt-1 w-full{{ $errors->has('guard_name') ? ' border-red-400' : '' }}"
                                type="text" name="guard_name" value="{{ old('guard_name') }}" />
                        </div>
